...
def define_components(m):
    ...
    m.RPS_Enforce = Constraint(m.PERIODS, rule=lambda m, per:
        ( # RE Sources
            sum(
                m.DispatchProjByFuel[proj, tp, f] 
                for proj, tp, f in m.PROJ_FUEL_DISPATCH_POINTS
                if m.tp_period[tp] == per and proj in m.FUEL_BASED_PROJECTS and f in m.RPS_ENERGY_SOURCES
            )
            +
            sum(
                m.DispatchProj[proj, tp] 
                for proj, tp in m.PROJ_DISPATCH_POINTS
                if m.tp_period[tp] == per and proj in m.NON_FUEL_BASED_PROJECTS 
                    and m.g_energy_source[m.proj_gen_tech[proj]] in m.RPS_ENERGY_SOURCES
            )
            +
