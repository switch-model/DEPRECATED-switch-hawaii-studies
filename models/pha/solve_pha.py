...
def main():
    ...
    parser.add_argument('--dr_share', type=float)
...
def solve(
    ...
    batteries=True,
    demand_response_simple=True, dr_share=0.3,
    ev=True, 
    ...
    dat_file = os.path.join(inputs_dir, 'complete_inputs_{t}.dat'.format(t=tag))

    if action = "write_dat_file":
        # run through progressive hedging system
        tic()
        log("writing reference model data to {}... ".format(dat_file))
        save_inputs_as_dat(switch_model, switch_instance, save_path=dat_file)
        toc()

    if action = "solve":
        setup_results_dir()

        # store old value of tag
        orig_tag = tag
        
        for i in range(4):
            tag = orig_tag + '_' + str(i).zfill(4)
