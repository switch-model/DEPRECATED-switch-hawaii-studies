...
data_dir = os.path.join("inputs_tiny", "pha")
...
for s in range(n_scenarios):
    ...
    # )
    with open(
        os.path.join(data_dir, "fuel_supply_curves_{s}.dat".format(s=str(s).zfill(n_digits))),
        "w"
    ) as f:
        f.write('param: ' + '\t'.join(sort_cols(headers)[3:]) + ' :=\n') # omit headers for index cols
        for r in fuel_data:
